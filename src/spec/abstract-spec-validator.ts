import type { AbstractSpec, ColumnRef } from "./abstract-spec.js";
...
function isUnknownTable(columnRef: ColumnRef): boolean {
  return columnRef.table.trim().toLowerCase() === "unknown_table";
}

function isRawFederatedToken(columnRef: ColumnRef): boolean {
  return /federated\.[^.]+\./i.test(columnRef.column);
}

export class AbstractSpecValidator {
  public validate(spec: AbstractSpec): AbstractSpecValidationReport {
    const issues: AbstractSpecValidationIssue[] = [];
    ...
    for (const [pageIndex, page] of spec.dashboard_spec.pages.entries()) {
      for (const [visualIndex, visual] of page.visuals.entries()) {
        ...
        const axisEntries: Array<{ key: string; ref: ColumnRef }> = [];
        if (axes.x !== undefined) {
