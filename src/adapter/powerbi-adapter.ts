...
export class PowerBIAdapter implements ITargetAdapter {
  public constructor(private readonly deployConfig: PowerBIDeployConfig = {}) {}

  public validate(spec: AbstractSpec): AdapterValidationResult {
    const errors: string[] = [];
  ...
  public async build(spec: AbstractSpec, outputDir: string): Promise<AdapterBuildResult> {
    const qualityPipeline = new PowerBIQualityPipeline();
    const quality = qualityPipeline.prepareSpec(spec);
    if (!quality.valid) {
  ...
  public async deploy(buildResult: AdapterBuildResult): Promise<AdapterDeployResult> {
    ...
    const baseResult: AdapterDeployResult = {
      success: true,
