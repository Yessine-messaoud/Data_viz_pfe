...
import type { AbstractSpec, ColumnRef, MeasureDef } from "../spec/abstract-spec.js";
...
function scoreFactCandidate(
  ...
  const profile = (spec.data_lineage.full_table_profiles ?? []).find(
    (entry) => normalizeTableName(entry.table_name) === normalizeTableName(tableName),
  );
  const columns = profile?.columns ?? [];

  const fk = columns.filter((column) => /(?:key|_key|keyid|_id|id|linekey)$/i.test(column.name)).length;
  const joinMany = spec.data_lineage.joins.filter(
    (join) => normalizeTableName(join.left_table) === normalizeTableName(tableName),
  ).length;
  const measure = columns.filter(
    (column) =>
      /(amount|qty|quantity|price|cost|sales|profit|revenue|margin)/i.test(column.name) &&
      /(integer|int|real|float|double|number|numeric|decimal)/i.test(column.type),
  ).length;
...
function inferFactTableFromSpec(spec: AbstractSpec): string | undefined {
...
function findAllFunctions(expression: string): string[] {
...
function columnRefExists(columnRef: ColumnRef, knownColumns: Set<string>, knownTables: Set<string>): boolean {
...
function autoFixExpression(expression: string): { expression: string; changed: boolean; reason?: string } {
...
function autoFixSpec(spec: AbstractSpec): { spec: AbstractSpec; warnings: PowerBIQualityWarning[] } {
  const warnings: PowerBIQualityWarning[] = [];

  const fixMeasure = (measure: MeasureDef): MeasureDef => {
...
function validateModel(spec: AbstractSpec): PowerBIQualityIssue[] {
  const issues: PowerBIQualityIssue[] = [];
  const knownTables = new Set(
    [...spec.data_lineage.tables, ...(spec.data_lineage.full_tables ?? [])].map((table) => normalizeTableName(table.name)),
  );
...
function validateDax(spec: AbstractSpec): PowerBIQualityIssue[] {
  const issues: PowerBIQualityIssue[] = [];
  const knownColumnTokens = new Set<string>();
  for (const usage of spec.data_lineage.columns_used) {
...
function validatePbixBytes(pbixBytes: Uint8Array): PowerBIQualityIssue[] {
  const issues: PowerBIQualityIssue[] = [];
...
export class PBIXTemplate {
  public createBaseArchive(spec: AbstractSpec): AdmZip {
...
export class PowerBIQualityPipeline {
  public prepareSpec(spec: AbstractSpec): PowerBIQualityResult {
  ...
  public validatePbix(pbixBytes: Uint8Array): PowerBIQualityIssue[] {
