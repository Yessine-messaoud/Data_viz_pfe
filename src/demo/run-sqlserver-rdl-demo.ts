...
import type { AbstractSpec, DashboardPage } from "../spec/abstract-spec.js";
...
async function pickWorkbookFile(inputDir: string, preferredToken: string): Promise<string> {
...
async function testSqlConnection(server: string, database: string, integratedSecurity: boolean): Promise<void> {
...
function keepSingleChart(spec: AbstractSpec, preferredWorksheetName?: string): { spec: AbstractSpec; selectedWorksheet: string } {
  ...
  const filteredPages: DashboardPage[] = spec.dashboard_spec.pages
    .map((page) => ({
      ...page,
...
export async function runSqlServerRdlDemo(workspaceRoot: string): Promise<{
  workbookPath: string;
  pbixPath: string;
  rdlPath: string;
  sqlServer: string;
