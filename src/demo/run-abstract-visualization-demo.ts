...
import type { ParsedWorkbook } from "../tableau/interfaces.js";
...
async function loadWorkbook(filePath: string): Promise<LoadedWorkbook> {
...
export async function runAbstractVisualizationDemo(workspaceRoot: string): Promise<string> {
  ...
  const spec = await createAbstractSpecFromParsedWorkbookHybrid(
    loaded.parsedWorkbook,
    loaded.workbookRaw,
    loaded.workbookName,
    loaded.extraction,
  );
