...
import type { ColumnRef, SemanticModel } from "../spec/abstract-spec.js";
...
function sanitizeSuggestedExpression(expression: string): string {
...
function toColumnLabel(column: ColumnRef): string {
...
function buildDeterministicResult(base: SemanticModel, context: SemanticEnrichmentContext): SemanticEnrichmentResult {
...
export class HybridSemanticEnricher implements SemanticEnricher {
  ...
  public constructor(options: HybridSemanticEnricherOptions = {}) {
    const resolved: {
      model: string;
      baseUrl: string;
      provider: "mistral" | "none";
  ...
  private getLocalLlamaEndpoint(): string {
    return process.env.LOCAL_LLM_BASE_URL?.trim() || "http://127.0.0.1:11434";
  }

  private getLocalLlamaModel(): string {
    return process.env.LOCAL_LLM_MODEL?.trim() || "llama3";
  }

  private buildCloudMessages(base: SemanticModel, context: SemanticEnrichmentContext): CloudMessage[] {
    const payload = {
    ...
    return [
      {
        role: "system",
        content:
          "You are a semantic BI model assistant. Return only valid JSON with keys: renamedDimensions, suggestedMeasures, disambiguationNotes.",
      },
      {
        role: "user",
        content:
          "Using this semantic context, propose conservative semantic enrichments. JSON schema: {\"renamedDimensions\":[{\"from\":string,\"to\":string,\"confidence\":number}],\"suggestedMeasures\":[{\"name\":string,\"expression\":string,\"confidence\":number}],\"disambiguationNotes\":[string]}. Input:\n" +
          JSON.stringify(payload),
      },
    ];
  }

  private parseCloudResult(content: string): SemanticEnrichmentResult {
    ...
    const parsed = JSON.parse(jsonText) as {
      renamedDimensions?: Array<{ from?: string; to?: string; confidence?: number }>;
      suggestedMeasures?: Array<{ name?: string; expression?: string; confidence?: number }>;
    ...
    const suggestedMeasures = (parsed.suggestedMeasures ?? [])
      ...
      .map((item) => ({
        ...
        expression: sanitizeSuggestedExpression(item.expression as string),
        confidence: clampConfidence(item.confidence, 0.7),
      }));

    const disambiguationNotes = (parsed.disambiguationNotes ?? []).filter(
  ...
  private async enrichWithMistral(base: SemanticModel, context: SemanticEnrichmentContext): Promise<SemanticEnrichmentResult> {
  ...
  private async enrichWithLocalLlama3(base: SemanticModel, context: SemanticEnrichmentContext): Promise<SemanticEnrichmentResult> {
  ...
  public async enrich(base: SemanticModel, context: SemanticEnrichmentContext): Promise<SemanticEnrichmentResult> {
    ...
    try {
      ...
      try {
        ...
        return {
          renamedDimensions:
            localResult.renamedDimensions.length > 0 ? localResult.renamedDimensions : deterministic.renamedDimensions,
          suggestedMeasures:
            localResult.suggestedMeasures.length > 0 ? localResult.suggestedMeasures : deterministic.suggestedMeasures,
          disambiguationNotes: [
