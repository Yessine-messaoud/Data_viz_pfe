import type { ColumnRef, DataLineageSpec, SemanticModel } from "../spec/abstract-spec.js";
import type {
  GraphEdge,
  GraphNode,
  SemanticGraph,
...
function columnId(column: ColumnRef): string {
  return `column:${column.table}.${column.column}`;
}

export class AdditiveSemanticGraphBuilder implements SemanticGraphBuilder {
  public constructor(private readonly repository: SemanticGraphRepository) {}

  public async build(model: SemanticModel, lineage: DataLineageSpec): Promise<SemanticGraph> {
...
function deduplicateGraph(graph: SemanticGraph): SemanticGraph {
