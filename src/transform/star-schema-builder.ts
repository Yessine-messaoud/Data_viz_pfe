...
import type { StarBridgeTable, StarDimensionTable, StarSchemaBuilder, StarSchemaModel } from "./interfaces.js";

function looksLikeDateColumn(columnName: string): boolean {
  const lower = columnName.toLowerCase();
  return lower.includes("date") || lower.includes("month") || lower.includes("year") || lower.includes("quarter");
}

export class SemanticStarSchemaBuilder implements StarSchemaBuilder {
  public build(model: SemanticModel, lineage: DataLineageSpec): StarSchemaModel {
    const factTable = model.fact_table;

    const dimensionMap = new Map<string, StarDimensionTable>();
    ...
    const hasDateInDimensions = dimensions.some((dimension) =>
      dimension.sourceColumns.some((sourceColumn) => looksLikeDateColumn(sourceColumn.column)),
    );
