import type { ParsedColumn, ParsedDatasource, ParsedWorkbook } from "./interfaces.js";
...
function tableNameFromEntity(value: string): string {
  const normalized = normalizeTableName(value).replace(/!data$/i, "");
  return `${normalized}_data`;
}

function collectNamedConnectionTables(workbook: JsonRecord): Map<string, Set<string>> {
...
function inferTableFromField(fieldName: string, tableCandidates: string[]): string | undefined {
...
function decodeFederatedToken(token: string, tableCandidates: string[]): string {
...
function resolveDatasourceTables(datasource: ParsedDatasource, namedConnectionTables: Set<string>): string[] {
...
function resolveColumns(columns: ParsedColumn[], tableCandidates: string[]): ParsedColumn[] {
...
export class FederatedDatasourceResolver {
  public resolve(parsedWorkbook: ParsedWorkbook, workbook: JsonRecord): ParsedWorkbook {
    ...
    const resolvedDatasources = parsedWorkbook.datasources.map((datasource) => {
      ...
      const namedConnectionTables = federatedId.length > 0 ? namedConnectionMap.get(federatedId) ?? new Set<string>() : new Set<string>();
    ...
    const resolveTokens = (tokens: string[]): string[] =>
      tokens
