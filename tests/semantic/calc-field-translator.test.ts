...
import {
  buildGroqCalcMessages,
  validateDaxExpressionForRetry,
  type GroqTranslationContext,
} from "../../src/demo/parsed-workbook-to-abstract.js";
...
test("FIX3 prompt LLM inclut schema complet, mesures existantes et few-shot", () => {
  ...
  const context: GroqTranslationContext = {
  ...
  const payload = JSON.parse(messages[1]?.content ?? "{}") as {
    availableTablesAndColumns?: Record<string, string[]>;
    existingMeasures?: string[];
    fewShot?: Array<{ tableau: string; dax: string }>;
  };
