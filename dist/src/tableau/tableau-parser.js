...
function readTextArray(node, key) {
    const raw = node?.[key];
...
function extractWorksheets(root) {
...
function extractCalculatedFields(root) {
    const datasourcesNode = asRecord(root["datasources"]);
    const datasourceItems = asArray(datasourcesNode?.["datasource"]);
...
function extractDashboards(root) {
