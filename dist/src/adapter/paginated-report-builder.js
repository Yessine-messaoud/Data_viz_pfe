...
export const PAGINATED_RDL_CONSTRAINTS = [
    {
        id: "RDL-001",
...
export function validatePaginatedRdl(xml, mode = "local") {
    const issues = [];
...
function validateBusinessConstraints(xml, constraints) {
    const issues = [];
...
export function validateGeneratedRdlShape(xml, mode = "local", options = {}) {
    const issues = [];
...
function formatColumnRef(column) {
    if (column === undefined) {
        return "-";
    }
...
function deriveSqlDataset(spec) {
    ...
    const fallbackColumns = Array.from(new Map(spec.data_lineage.columns_used.map((usage) => [
...
function buildSqlConnectString(config) {
...
function buildSqlDataSourcesXml(config) {
    return `
  <DataSources>
...
function buildSqlTablix(datasetName, fields) {
...
function renderVisualLines(page) {
...
function buildBodyReportItems(spec, options) {
...
export function buildPaginatedRdl(spec, options) {
    ...
    const sqlTablix = mode === "sqlserver" && sqlDataSets !== undefined
        ? buildSqlTablix(sqlDataSets.datasetName, sqlDataSets.fields)
        : "";
    return `<?xml version="1.0" encoding="utf-8"?>
<Report xmlns="http://schemas.microsoft.com/sqlserver/reporting/2016/01/reportdefinition">
  <AutoRefresh>0</AutoRefresh>
  <Description>${escapeXml(mode === "sqlserver"
        ? `${reportName} generated in SQL Server mode with external datasource.`
        : `${reportName} generated in local mode without external datasource.`)}</Description>
  <CustomProperties>
    <CustomProperty>
...
export async function writePaginatedRdl(spec, outputDir, validationOptions = {}) {
    ...
    await writeFile(`${filePath}.validation.json`, JSON.stringify(validation, null, 2), "utf8");
    return filePath;
}
export async function writePaginatedRdlSqlServer(spec, outputDir, config, validationOptions = {}) {
    const qualityPipeline = new PowerBIQualityPipeline();
    const quality = qualityPipeline.prepareSpec(spec);
    if (!quality.valid) {
        const details = quality.issues.map((issue) => `${issue.stage}:${issue.code}`).join(", ");
