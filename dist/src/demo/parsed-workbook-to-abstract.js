...
const KPI_WORKSHEET_MEASURE_MAP = {
...
function parseResolvedToken(token) {
...
function inferColumnRef(token, parsedWorkbook) {
    const clean = sanitizeToken(token);
...
function inferVisualType(worksheet, mapper) {
...
export function inferFactTableFromSourceTables(sourceTables, joins) {
...
function pickFallbackAxisColumns(parsedWorkbook) {
    const allColumns = parsedWorkbook.datasources.flatMap((datasource) => datasource.columns);
    const firstDatasource = parsedWorkbook.datasources[0];
    const fallbackTable = firstDatasource?.tables[0]?.name ?? "unknown_table";
    const dimensionCandidate = allColumns.find((column) => !NUMERIC_TYPES.has((column.dataType ?? "").toLowerCase()));
...
function extractFormulaSources(formula, parsedWorkbook) {
...
function inferJoinsFromSourceTables(sourceTables) {
    const parsedJoinInputs = [];
    for (let leftIndex = 0; leftIndex < sourceTables.length; leftIndex += 1) {
        const left = sourceTables[leftIndex];
        if (left === undefined) {
            continue;
        }
        const leftColumns = new Map(left.columns.map((column) => [normalizeColumnName(column.name), sanitizeToken(column.name)]));
        for (let rightIndex = leftIndex + 1; rightIndex < sourceTables.length; rightIndex += 1) {
            const right = sourceTables[rightIndex];
            if (right === undefined) {
                continue;
            }
            const rightColumns = new Map(right.columns.map((column) => [normalizeColumnName(column.name), sanitizeToken(column.name)]));
            const sharedKeys = Array.from(leftColumns.keys()).filter((column) => rightColumns.has(column));
            if (sharedKeys.length === 0) {
                continue;
            }
            const rankedKey = sharedKeys.find((column) => /(^id$|_id$|id$|_key$|key$)/.test(column)) ?? sharedKeys[0] ?? "";
...
function buildRelationshipsFromJoins(joins) {
    const relationships = [];
...
function parseCsvPreview(bytes) {
...
function parseBinaryHeadPreview(bytes) {
    const result = [];
...
function extractSourceTables(parsedWorkbook, extraction) {
    const diagnostics = {
        logs: [],
        warnings: [],
    };
    ...
    for (const table of tables) {
        diagnostics.logs.push(`table ${table.table_name}: columns=${table.columns.length}, source=${table.source}, preview_rows=${table.sample_data.length}`);
...
function findMissingVisualColumns(dashboardSpec, sourceTables) {
...
export function buildDashboardSpecFromParsedWorkbook(parsedWorkbook) {
    ...
    const resolveWorksheetsForDashboard = (dashboardName) => {
    ...
    if (parsedWorkbook.dashboards.length > 0) {
        pages = parsedWorkbook.dashboards.map((dashboard) => {
            ...
            const resolvedWorksheets = dashboardWorksheets.length > 0
                ? dashboardWorksheets
...
export function buildSemanticModelFromParsedWorkbook(parsedWorkbook) {
    const allColumns = parsedWorkbook.datasources.flatMap((datasource) => datasource.columns);
...
export function buildLineageFromParsedWorkbook(parsedWorkbook, dashboardSpec) {
    const tables = parsedWorkbook.datasources.flatMap((datasource, index) => datasource.tables.map((table, tableIndex) => ({
...
function isComplexCalcFormula(formula) {
    return /\b(fixed|window|running|countd|if|case)\b/i.test(formula);
}
function buildSemanticEnrichmentContext(parsedWorkbook, model, lineage) {
    const glossary = Object.fromEntries(model.glossary
    ...
    for (const usage of lineage.columns_used) {
        ...
        const tables = byColumnName.get(key) ?? new Set();
...
function sanitizeMistralDaxExpression(expression) {
...
export function validateDaxExpressionForRetry(expression) {
...
function resolveMistralBaseUrl() {
...
function resolveLocalLlamaEndpoint() {
    return process.env.LOCAL_LLM_BASE_URL?.trim() || "http://127.0.0.1:11434";
}
function resolveLocalLlamaModel() {
    return process.env.LOCAL_LLM_MODEL?.trim() || "llama3";
}
export function buildCloudCalcMessages(context) {
    ...
    return [
        {
            role: "system",
            content: "Translate Tableau formulas to DAX. Return strict JSON object only with key daxExpression. Use Table[Column] format, never Table.Column. Reuse existing measures and avoid duplicates.",
        },
        {
            role: "user",
...
export function parsedWorkbookToTableColumns(parsedWorkbook) {
    ...
    return Object.fromEntries(Array.from(byTable.entries()).map(([table, cols]) => [table, Array.from(cols.values()).sort()]));
}
async function requestCloudDaxTranslation(context) {
...
async function requestLocalLlamaDaxTranslation(context, previousError) {
    try {
        const endpoint = `${resolveLocalLlamaEndpoint().replace(/\/$/, "")}/api/generate`;
        const prompt = "Translate Tableau formula to DAX. Return strict JSON with key daxExpression only. Use Table[Column] format. " +
            JSON.stringify({
                formula: context.formula,
                previousError,
...
export function runSemanticLayerFromParsedWorkbook(parsedWorkbook, extraction) {
    ...
    for (const [visualId, visualLineage] of Object.entries(data_lineage.visual_column_map)) {
        ...
        const joinsUsed = inferredJoins
            .filter((join) => usedTables.has(normalizeTableName(join.left_table)) && usedTables.has(normalizeTableName(join.right_table)))
            .map((join) => join.id);
...
export async function runSemanticLayerFromParsedWorkbookHybrid(parsedWorkbook, extraction) {
    ...
    const context = buildSemanticEnrichmentContext(parsedWorkbook, calcTranslation.semanticModel, deterministic.data_lineage);
...
export function buildAbstractSpecPivot(semanticLayer, workbookRaw, workbookName) {
    ...
    }, semanticLayer.semantic_model, semanticLayer.data_lineage);
    return {
        ...base,
...
export function createAbstractSpecFromParsedWorkbook(parsedWorkbook, workbookRaw, workbookName, extraction) {
...
export async function createAbstractSpecFromParsedWorkbookHybrid(parsedWorkbook, workbookRaw, workbookName, extraction) {
