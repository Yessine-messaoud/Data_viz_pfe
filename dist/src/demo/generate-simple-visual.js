...
function renderVisualList(spec) {
...
export function createTablePreviewHTML(tables) {
    return tables
        .map((table) => {
...
function renderSchemaGraph(spec) {
    ...
    const tableNames = profileNames.length > 0
        ? profileNames
...
function buildLayerEvidenceBlocks(spec) {
...
function renderLayerEvidence(spec) {
...
function renderPipelineStepStatus(spec, proof) {
    ...
    return orderedSteps
        ...
        const samplePayload = stepName === "7) LLM Calc Translation"
...
function parseLlmRuntimeStatus(spec) {
...
function computeLayerStatuses(spec) {
...
async function collectArtifactStatuses(outputDir) {
    const artifacts = [];
    const expected = [
...
async function loadExtractionProof(outputDir) {
...
function renderExtractionProof(extractionProof) {
...
function renderLlmStatusPanel(spec) {
    const llm = parseLlmRuntimeStatus(spec);
...
function renderHtml(spec, proof) {
...
export async function generateSimpleVisualFromAbstractJson(workspaceRoot) {
